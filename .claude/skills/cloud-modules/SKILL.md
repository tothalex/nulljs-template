...
// Plain objects/strings are serialized for you; Buffers pass through as-is. A handler
// with a `schema` receives the payload parsed+typed; without one it gets a Buffer.
await send("event-name", { some: "data" });
